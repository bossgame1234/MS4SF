...
deviceMainController.controller('PlotWithDeviceController',['$scope','$http','$state', '$rootScope','deviceService','$ionicPopup',
    function ($scope, $http,$state, $rootScope,deviceService,$ionicPopup) {
        ...
        $http.get("../../public/device/"+$rootScope.plotId+"/edit").success(function (data) {
            $scope.devices = data;
        });
        ...
        $scope.deleteDevice = function(id){
            ...
            if (answer) {
                $http.get("../../public/device/destroy/"+id).success(function () {
                    $ionicPopup.alert({
                        title: 'Success',
                        template: 'success'
                    });
...
deviceMainController.controller('sensorMonitoringSummaryController',['$scope', '$interval','$http','$location', '$rootScope',
    ...
    var id =$rootScope.DeviceId;
        ...
        $http.get("../../public/sensor/"+id).success(function(data){
            $scope.SensorID = data.id;
            $rootScope.showSensor = true;
            $http.get("../../public/lightSummary/"+$scope.SensorID).success(function(data){
                $scope.light = data;
            });
            $http.get("../../public/humiditySummary/"+$scope.SensorID).success(function(data){
                $scope.humidity = data;
            });
            $http.get("../../public/soilMoistureSummary/"+$scope.SensorID).success(function(data){
                $scope.soilMoisture = data;
            });
            $http.get("../../public/temperatureSummary/"+$scope.SensorID).success(function(data){
                $scope.temperature = data;
            });
            $http.get("../../public/daily/"+$scope.SensorID).success(function(data){
                $scope.hours = data;
            });
            $http.get("../../public/currentEnvironmentValue/"+$scope.SensorID).success(function(data){
                $scope.current = data;
            });
        });
    $interval(function(){
...
deviceMainController.controller('smartMonitoringController',['$scope','$state','$http', '$rootScope','farmService','ionicMaterialInk','$ionicPopup',
    function ($scope,$state, $http, $rootScope,farmService,ionicMaterialInk,$ionicPopup) {
        ...
        $scope.saveFarmID = function(id) {
           for(var i=0;i<$scope.farms.length;i++){
               if($scope.farms[i].id==id){
                   ...
                   $http.get("../../public/plot/" + id + "/edit").success(function (data) {
                       $scope.plots = data;
                       $scope.loadPlot = true;
                   });
               }
        ...
        $scope.savePlotID = function(id,name) {
            for(var i=0;i<$scope.plots.length;i++) {
                if ($scope.plots[i].id == id) {
                    ...
                    $http.get("../../public/device/" + id + "/edit").success(function (data) {
                        $scope.devices = data;
        ...
        $scope.saveDeviceID= function(id,device_id){
            if(id!=null) {
                $rootScope.DeviceId = id;
                $rootScope.Device_id = device_id;
                $rootScope.SelectedDevice = true;
